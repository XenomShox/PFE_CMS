(() => {
    class DataDocument {
        ...
        constructor(Name, Schema) {
            ...
            let $this = this;
            this.SaveButton = $(
                `<button class="ml-auto mr-2 btn btn-success btn-border">Save Data</button>`
            );
            this.QuiteButton = $(
                `<button class="ml-auto mr-2 btn btn-border btn-danger btn-icon"><i class="fa fa-window-close"></i></button>`
            );
            this.Inner = $(
                `<div class="document col-9 col-md-7 mx-auto"></div>`
            ).append(
                $(`<div class="card"></div>`)
                    .append(
                        $(
                            `<div class="card-header row"><h3 class="ml-2 card-title">${Name}</h3></div>`
        ...
        FillInputs(inputs, data) {
            if (inputs.InputQuery !== undefined) {
                inputs.InputQuery.val(
                    data[inputs.Key] === undefined
                        ? null
                        : inputs.InputQuery.attr("type") !== "date"
                        ? data[inputs.Key]
                        : this.DateInput(data[inputs.Key])
                ).trigger("change");
                ...
                if (data[inputs.Key])
                    data[inputs.Key].forEach((el, i) => {
                        let x = this.CreateInput(
                                inputs.SubQuery,
                                inputs.Value.length,
                                inputs.type
                            ),
                            DeleteButoon = $(
                                `<button class="btn btn-icon btn-round btn-border btn-danger mr-2 ml-auto"><i class="fa fa-trash"></i></button>`
                            ),
                            $this = this;
                ...
                inputs.Value.forEach((l) => {
                    this.FillInputs(
                        l,
                        data[inputs.Key] === undefined ? {} : data[inputs.Key]
                    );
                });
        }
        FillBody(data) {
        ...
        CreateBody(Schema) {
            ...
            for (let key in Schema) {
                if (Schema.hasOwnProperty(key)) {
                    this.Body.data.push(
                        this.CreateInput(this.Body.Query, key, Schema[key])
                    );
                }
            }
        }
        CreateInput(Query, key, schemaElement) {
            let Input = {
                ...
                label = $(
                    `<label class="mb-2 d-flex col-12" for="${key}">${key}</label>`
                );
            ...
            if (schemaElement.type !== undefined) {
                ...
                let button = $(
                        `<div class="btn btn-success btn-border ml-auto"><i class="fa fa-plus"></i>add ${Input.Key}</div>`
                    ),
                    $this = this;
                label.append(button);
                button.click(function () {
                    let index = Input.Value.findIndex(
                        Object.is.bind(null, undefined)
                    );
                    ...
                    DeleteButoon.click(function () {
                        Input.Value[
                            Input.Value.indexOf(x)
                        ].InputQuery.parent().remove();
                        delete Input.Value[Input.Value.indexOf(x)];
                    });
                    Input.Value[index] = x;
                    index = Input.Value.findIndex(
                        Object.is.bind(null, undefined)
                    );
                    index = index < 0 ? Input.Value.length : index;
                });
        ...
        Input(type, key) {
            switch (type) {
                case "ObjectId":
                    return `<input class="form-control" placeholder="ObjectId" type="text" name="${key}">`;
                case "String":
                    return `<input class="form-control" placeholder="Text" type="text" name="${key}">`;
                case "Number":
                    return `<input class="form-control" placeholder="Number" type="number" name="${key}">`;
                case "Date":
                    return `<input class="form-control" placeholder="Date" type="date" name="${key}">`;
                case "Boolean":
                    return `<select class="form-control" name="${key}"><option value=true>true</option><option value=false>false</option></select>`;
            }
        }
    }
    class DataTable {
        ...
        CreateSkeleton() {
            let AddButton = $(
                    `<button class="btn btn-round btn-border btn-secondary btn-icon ml-auto mr-2 my-auto"><i class="fa fa-plus"></i></button>`
                ),
                $this = this,
                EditButton = $(
                    '<button disabled class="btn btn-round btn-border btn-success btn-icon  mr-2 my-auto"><i class="fa fa-edit"></i></button>'
                ),
                RemoveButton = $(
                    '<button disabled class="btn btn-round btn-border btn-danger btn-icon mr-4 my-auto"><i class="fa fa-trash"></i></button>'
                );
            ...
            this.#Container
                ...
                .append(
                    $('<div class="card-body table-responsive"></div>').append(
                        this.#Table
                    )
                );
            ...
            EditButton.click(function () {
                $this.Edit(
                    $this.#dataTable,
                    $this.#dataTable.rows(".selected").indexes()
                );
            });
            ...
            for (let key in SubSchema) {
                if (SubSchema.hasOwnProperty(key)) {
                    if (
                        ...
                        SubSchema[key] instanceof Array
                    )
                        columns.push({
                            data: name + "[, ]",
                            title: name,
                        ...
                        typeof SubSchema[key] === "object" &&
                        SubSchema[key].type === undefined
                    )
                        this.nestedObjectColumns(
                            SubSchema[key],
                            columns,
                            name + "." + key
                        );
        ...
        async Edit(table, indexes) {
            for (let i = 0; i < indexes.length; i++) {
                ...
                await this.#DataDocument
                    .show(row.data(), this.#DataUrl)
                    .then((data) => {
                        data = data.value[0];
        ...
        Remove(dataTable, indexes) {
            let removelist = [],
                list = "";
            for (let i = 0; i < indexes.length; i++) {
                let row = dataTable.row(indexes[i]),
                    id = dataTable.row(indexes[i]).data()["_id"];
                list += id + ", ";
                    ...
                    preConfirm: async () => {
                        ...
                        for (let i = 0; i < removelist.length; i++) {
                            await $.ajax(this.#DataUrl, {
                                ...
                                timeout: 3000,
                                type: "DELETE",
                                data: { _id: removelist[i].id },
                            }).catch((reason) => {
                                b = false;
                                $(removelist[i].row.node()).removeClass(
                                    "selected"
                                );
                        ...
                        Swal.insertQueueStep({
                            ...
                            text:
                                (b
                                    ? "All"
                                    : removelist.length > 0
                                    ? "An error Happened but Some of the"
                                    : "Error Happened None of the") +
    ...
    class DataBaseManager {
        ...
        constructor(Container, ApiUrl) {
            ...
            this.FetchData()
                .then((data) => {
                    $(".content").prepend(
                        (DataDocument.Container = $(
                            "<div class='Hidden Container'></div>"
                        ))
                    );
                    this.CreateDataTables(data);
                    this.#Container.removeClass(
                        "is-loading is-loading-secondary is-loading-lg"
                    );
                })
                .catch((reason) => {
                    console.log(reason);
        ...
        CreateDataTables(data) {
            ...
            data.forEach((el, i) => {
                let Container = $(
                    '<div class="DataTable card col-11 mx-auto mb-5"></div>'
                );
                this.#DataTables[i] = {
    ...
    $.fn.DataBase = function (ApiUrl) {
        return new DataBaseManager(this, ApiUrl);
    };
})();
