(()=>{
    class Schema{
        ...
        CreateSkeleton(data){
            ...
            this.#Skeleton={
                Query:$(`<div class="Schema card"></div>`).append(Header.Query).append(Body.Query).append(Footer.Query),
                Header:Header,
                Body:Body,
                Footer:Footer,
                Edit:false
        ...
        CreateHeader(Name) {
            let $this=this,
                ...
                Header={
                    Query:$(`<div class="row card-header"></div>`).append(NameQuery).append(EditButton).append(DeleteButton),
                    Name:Name,
                };
            EditButton.click(function(){$this.EditOrAdd(EditButton)});
            DeleteButton.click(function(){$this.#schemaManager.DeleteSchema($this.#index)});
            NameQuery.change(()=>{
        ...
        CreateBody(Schema) {
            let Body={ Query:$(`<div class="card-body">
        ...
        CreateFooter() {
            let Footer={
                ...
                Cancel:$('<button class="ml-2 btn btn-danger btn-border">Cancel</button>')
            },$this=this;
            Footer.Query.append(Footer.Save).append(Footer.Cancel);
            Footer.Save.click(function () {$this.Save();});
            Footer.Cancel.click(function () {$this.Reset();});
            return Footer;
        }
        EditOrAdd(button) {
            if(this.#Skeleton.Edit){
                let Element=this.CreateElement("key"+(this.#Skeleton.Body.Elements.length+1), {type:"String"},this.#Skeleton.Body);
        ...
        CreateElement(Key, schemaElement,Parent) {
            let AddButton=$('<button class="my-auto ml-auto btn btn-border btn-icon btn-primary btn-xs btn-round"><i class="fas m-auto fa-plus fa-xs"></i></button>'),
                RemoveButton=$('<button class="my-auto ml-2 btn btn-border btn-icon btn-danger btn-xs btn-round"><i class="fas m-auto fa-trash fa-xs"></i></button>'),
                KeyInput=$(`<input class="col" type="text" value="${Key}" name="Key">`),
                SchemaHead=$(`<div class="row col-10 mr-auto"></div>`).append(KeyInput).append(":"),
            Element={
            ...
            if(notGroup){
                ...
                for(let key in schemaElement){
                    if(schemaElement.hasOwnProperty(key)){
                        if(key!=="type"){
                            let newElement=this.CreateOption(key,schemaElement[key],Element);
                            Element.Options.splice(Element.Options.indexOf(key),1);
                ...
                for(let key in schemaElement){
                    if(schemaElement.hasOwnProperty(key)){
                            let newElement=this.CreateElement(key,schemaElement[key],Element);
                            Element.Elements.push(newElement);
                            Element.CollapserQuery.append(newElement.Query);
                    }
                }
            ...
            Select.change(function () {
                let val=Select.val();
                Element.Type=val;
                if(val==="Group") {Element.Options=[];notGroup=false}
                else {Element.Options=$this.#schemaManager.GetOptions(val);notGroup=true}
            ...
            AddButton.click(function (event) {
                if(Element.CollapserQuery.hasClass("show"))event.stopPropagation();
                let newElement;
                if(notGroup){
                    let option=Element.Options.shift();
                    if(option===undefined) return;
                    newElement=$this.CreateOption(option,null,Element);
                    if(Element.Options.length===0) AddButton.addClass("Hidden");
                    Element.CollapserQuery.find("[name=option]").each((i,el)=>{
                        let val=$(el).val();
                        $(el).empty().append(`<option value="${val}" selected>${val}</option>`+$this.Option(Element.Options));
                    });
        ...
        CreateOption(Key,Value,Parent) {
            let Options=$(`<select class="col" name="option"><option value="${Key}" selected>${Key}</option></select>`),
                ...
                Element = {
                    Query: $(`<div class="Schema-Container"></div>`)
                        .append($('<div class="row Schema-Head"></div>')
            ...
            Options.change(function () {
                let val=Options.val(),nValues=$($this.Values(val,null,Parent));
            ...
            RemoveButton.click(function () {
                ...
                Parent.CollapserQuery.find("[name=option]").each((i,el)=>{
                    let val=$(el).val();
                    $(el).empty().append(`<option value="${val}" selected>${val}</option>`+$this.Option(Parent.Options));
                });
            })
            Values.change(function () {Element.Type=Values.val();});
            return Element;
        ...
        Option(options,key) {
            let Options="";
            options.forEach(el=>{
               Options+=`<option value="${el}" ${key===el?"selected":""}>${el}</option>`;
            });
            return Options;
        }
        Values(Key,Value,Parent){
            ...
            switch (Type) {
                case "Boolean": return `<select class="col" name="Value"><option value=true ${Value===true?"selected":""}>true</option><option value=false ${Value===false?"selected":""}>false</option></select>`;
                case "Number":return `<input class="col" type="number" value="${Value===null?0:Value}" name="Value">`;
                case "Array":
        ...
        Reset(){
            ...
            this.#schemaManager.Schemas[this.#index].Container.empty().append(this.#Skeleton.Query);
        }
    }
    class SchemaManager{
        #DataUrl;
        ...
        SchemaNames=[];
        Schemas=[];
        Default={required:"Boolean",unique:"Boolean",index:"Boolean",dropDups:"Boolean"};
        Types=[
