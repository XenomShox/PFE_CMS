...
module.exports = class Schemas {
    /*----------------------------attribute-------------------------*/
    static #SchemaModel = mongoose.model( 'Vinland_Schemas' ,
                                          mongoose.Schema( {
                                                           Name   : {
        ...
        Schemas.#SchemaModel.findOne( { Name : Name } , { Name : 1 , Schema : 1 , _id : 0 } , ( err , result ) => {
            if ( !err ) {
                if ( result !== null ) {
                    ...
                    callback( 404 , { Error : 'There is no Schema with this name : ' + Name } );
                }
            }
            else {
                callback( 500 , { Error : err } );
    ...
    static UpdateASchema ( Name , NewName , iSchema , callback ) {
        try {
            ...
            Schemas.#SchemaModel.replaceOne( { Name : Name } , { Name : NewName , Schema : iSchema } , ( error ,
                result ) => {
                if ( error ) {
                    if ( error.code === 11000 ) {
                        callback( 409 , { Error : 'duplicate Name' } );
                    ...
                    callback( 404 , { Error : `Couldn't find ${ Name } to Replace it with ${ NewName }` } );
                }
                else {
