...
module.exports = class Schemas {
    /*----------------------------attribute-------------------------*/
    static #SchemaModel = mongoose.model(
        "Vinland_Schemas",
        mongoose.Schema(
            {
                Name: {
    ...
    static GetAllSchemas(callback) {
        return Schemas.#SchemaModel.find(
            {},
            { Name: 1, Schema: 1, _id: 0 },
            (err, result) => {
                if (err) {
            ...
            (err, result) => {
                if (!err) {
                    if (result !== null) {
                        ...
                        callback(404, {
                            Error:
                                "There is no Schema with this name : " + Name,
                        });
                    }
                } else {
                    callback(500, { Error: err });
    ...
    static AddASchema(Name, iSchema, callback) {
        try {
            ...
            Schemas.#SchemaModel.create(
                { Name: Name, Schema: iSchema },
                (error) => {
                    if (error) {
                        if (error.code === 11000) {
                            callback(409, { Error: "duplicate Name" });
                        } else {
                            throw new Error(error);
                        }
                    } else {
                        callback(201, { SUCCESS: "Created" });
                    }
                }
            );
        } catch (Error) {
            if (Error.name === "TypeError") {
                callback(400, { Error: "Invalid Schema" });
            } else {
                callback(500, { Error });
            }
        }
    ...
    static DeleteASchema(Name, callback) {
        Schemas.#SchemaModel.deleteOne({ Name: Name }, (err, result) => {
            if (err) {
                ...
                callback(404, { Error: `Couldn't find ${Name}` });
            } else {
    ...
    static UpdateASchema(Name, NewName, iSchema, callback) {
        try {
            ...
            Schemas.#SchemaModel.replaceOne(
                { Name: Name },
                { Name: NewName, Schema: iSchema },
                (error, result) => {
                    if (error) {
                        if (error.code === 11000) {
                            callback(409, { Error: "duplicate Name" });
                        ...
                        callback(404, {
                            Error: `Couldn't find ${Name} to Replace it with ${NewName}`,
                        });
                    } else {
                        Schemas.#ApiManager.Models.UpdateModel(
                            Name,
                            NewName,
                            Schema
                        );
