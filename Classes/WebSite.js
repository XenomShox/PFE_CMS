    ...
    async LoadDataForTemplate ( data ) {
        try {
            ...
            for ( let i = 0 ; i < data.length ; i++ ) {
                Data[ data[ i ].name ] = await require( './PostManager' ).GetData( data[ i ] );
            }
            return Data;
        }
        catch ( e ) {
