...
class CategoriesManager {
    ...
    constructor () {
        this.#CategoriesModel = mongoose.model( 'Vinland_Category' , {
            ...
            Cover       : String ,
            Slug        : { type : String , required : true , unique : true , dropDups : true , index : true } ,
    ...
    async GetCategory ( Slug ) {
        let category = await this.#CategoriesModel.findOne( { Slug : Slug } ).exec();
        if ( !category ) return new Error( 'Category not found' );
        return category;
    }

    AddPost ( Id , PostID ) {
        return this.#CategoriesModel.updateOne( { '_id' : Id } , { $push : { Posts : PostID } } ).exec();
    }
