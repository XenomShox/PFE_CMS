...
module.exports = class FileManager {
    ...
    static ReadFile ( Path ) {
        return fs.readFile( path.join( __dirname , FileManager.Path , Path ) , 'utf-8' );
    }

    static WriteFile ( Path , data ) {
        return fs.writeFile( path.join( __dirname , FileManager.Path , Path ) , data , 'utf-8' );
    }

    static async UploadFile ( Path , File ) {
        return fs.readFile( File.path )
            .then( data => {
                return fs.writeFile( path.join( __dirname , FileManager.Path , Path , File.name ) , data , { flag : 'wx' } )
                    .catch( () => {
                        let extension = path.extname( File.name )
                        File.name     = path.basename( File.name , extension ) + ( new Date() ).getTime() + extension;
                        return fs.writeFile( path.join( __dirname , FileManager.Path , Path , File.name ) , data );
                    } )
            } )
            .then( () => {
                Console.log( {
    ...
    static UploadFiles ( multiple , Path , Files , callback ) {
        fs.readdir( path.join( __dirname , FileManager.Path , Path ) )
            .catch( () => FileManager.CreateFolder( path.dirname( Path ) , path.basename( Path ) ) )
            .then( () => {
                if ( multiple ) {
                    let promises = [];
    ...
    static GetFileType ( FileName ) {
        ...
        switch ( ext ) {
            case '.swf':
                return { type : 'Flash' , icon : 'fab fa-foursquare' , extension : ext }
            //audio
            case '.aif':
            case '.cda':
            case '.mid':
            ...
            case '.m':
            case '.mm':
                return { type : 'C-language Family' , icon : 'fab fa-cuttlefish' , extension : ext }; // C-Languages
            case '.apk':
                return { type : 'Android-App' , icon : 'fab fa-android' , extension : ext };//Android
            case '.ipa':
                return { type : 'Ios-App' , icon : 'fab fa-app-store-ios' , extension : ext };//Ios App
            //fonts
            case '.fnt':
            case '.fon':
            case '.otf':
            ...
            //Icons
            case '.ico':
                return { type : 'Icon' , icon : 'fas fa-file-image' , extension : ext };
            ...
            case '.xlsm':
            case '.xlsx':
                return { type : 'Spreadsheet' , icon : 'fas fa-file-excel' , extension : ext }
            //System
            case '.bak':
            case '.cab':
            case '.cfg':
            ...
            case '.doc':
            case '.docx':
                return { type : 'Word' , icon : 'fas fa-file-word' , extension : ext };
            case '.pdf':
            ...
            case '.wpd':
            case '.odt':
                return { type : 'Text' , icon : 'fas fa-file-alt' , extension : ext };//Text File
            //PowerPoint
            case '.key':
            case '.odp':
            case '.ppt':
            case '.pps':
            case '.pptx':
                return { type : 'PowerPoint' , icon : 'fas fa-file-powerpoint' , extension : ext };
        }
    ...
    static GetFileStats ( Path , file ) {
        return fs.stat( path.join( __dirname , FileManager.Path , Path , file ) )
            .then( res => {
                return {
            ...
            .catch( reason => {
                ...
                return {
                    ...
                    Name : file ,
                    Type : { type : 'Corrupted' , icon : 'fas fa-file-medical-alt' } ,
                    Size : 0,
                };
            } );
    }
    ...
    static GetFilesTree ( Path = '' , callback , FoldersOnly = true ) {
        let result = {};
        fs.readdir( path.join( __dirname , FileManager.Path , Path ) , { withFileTypes : true } )
            .then( list => {
                let pending = list.length;
                if ( !pending ) return callback( 200 , result );
                list.forEach( file => {
                    if ( file.isDirectory() ) {
                        FileManager.GetFilesTree( Path + '/' + file.name , function ( status ,
                            res ) {
    ...
    static CreateFolder ( Path , FolderName , fullPath = false ) {
        return fs.mkdir( fullPath ? path.join( Path , FolderName ) :
    ...
    static DeleteFile ( Path , type , fullPath = false ) {
        let deleteFile;
        Path = fullPath ? Path : path.join( __dirname , FileManager.Path , Path.replace( /^(\\?files)?/g , '' ) );
    ...
    static MoveFile ( OldPath , NewPath , fullPathO = false , fullPathN = false ) {
        return fs.rename( fullPathO ? OldPath :
                          path.join( __dirname , FileManager.Path , OldPath.replace( /^(\\?files)?/g , '' ) ) ,
                          fullPathN ? NewPath : path.join( __dirname , FileManager.Path , NewPath ) )
            .then( () => {
                Console.log( {
    ...
    static CopyFile ( OldPath , NewPath ) {
        return fs.copy( path.join( __dirname , FileManager.Path , OldPath ) ,
                        path.join( __dirname , FileManager.Path , NewPath ) , fs.constants.COPYFILE_FICLONE )
            .then( () => {
                Console.log( {
    ...
    static async Decompress ( File ) {
        File            = path.join( __dirname , FileManager.Path , File.replace( /^(\\?files)?/g , '' ) );
        let Destination = path.join( path.dirname( File ) , path.basename( File , path.extname( File ) ) );
    ...
    static LoadJsonFile ( filename , fullPath = false ) {
        return fs.readFile( fullPath ? filename : path.join( __dirname , filename ) , "utf-8" )
            .then( ( file ) => {
    ...
    static WriteJsonFile ( filename , json , fullPath = false ) {
        return fs.writeFile( fullPath ? filename : path.join( __dirname , filename ) , JSON.stringify( json ) , "utf-8" )
    }
}
