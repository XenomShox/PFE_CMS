   ...
   static Path="../files";
    ...
    static UploadFiles(multiple,Path,Files,callback){
        fs.readdir(path.join(__dirname, FileManager.Path , Path))
            .catch(()=>{ return fs.mkdir(path.join(__dirname,FileManager.Path,Path)) })
            .then(()=>{
                if(multiple){
                    let promises=[];
    ...
    static CreateFolder(Path,FolderName,fullPath=false){
        return fs.mkdir(fullPath?path.join(Path,FolderName):path.join(__dirname,FileManager.Path,Path,FolderName));
    }
    ...
    static DeleteFile(Path,type,fullPath=false){
      ...
      return deleteFile
          .then(()=> {console.log(Path+" has been Deleted"); return {}})
          .catch(err=>{console.error(err); return err;});
   }
    static MoveFile(OldPath,NewPath,fullPathO=false,fullPathN=false){
        return fs.rename(fullPathO?OldPath:path.join(__dirname,FileManager.Path,OldPath.replace(/^(\\?files)?/g,"") ),fullPathN?NewPath:path.join(__dirname,FileManager.Path,NewPath));
   }
    ...
    static CopyFile(OldPath,NewPath){
        return fs.copy(path.join(__dirname,FileManager.Path,OldPath),path.join(__dirname,FileManager.Path,NewPath),fs.constants.COPYFILE_FICLONE);
   }
    ...
    static WriteJsonFile(filename,json,fullPath=false){ return fs.writeFile(fullPath?filename:path.join(__dirname,filename),JSON.stringify(json),"utf-8")}
}
