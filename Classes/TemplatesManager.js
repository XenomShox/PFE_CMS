...
class TemplatesManager {
    ...
    CreateTemplate(data) {
        return this.model.create(data).catch((reason) => {
            Console.error(reason, "Template creation");
        });
    }

    async getAppliedTemplate() {
        let templates = await this.model
            .find({})
            .sort("-applied")
            .limit(1)
            .exec();
    ...
    addTemplateFromZip(file) {
        let Template;
        return FileManager.Decompress(file).then((Destination) => {
            return FileManager.LoadJsonFile(
                path.join(Destination, "structure.json"),
                true
            )
                ...
                .then(async () => {
                    ...
                    promises.push(
                        FileManager.MoveFile(
                            path.join(Destination, "css"),
                            path.join("/css", Template.name),
                            true
                        )
                    );
                    promises.push(
                        FileManager.MoveFile(
                            path.join(Destination, "javascript"),
                            path.join("/javascript", Template.name),
                            true
                        )
                    );
                    promises.push(
                        FileManager.MoveFile(
                            ...
                            path.join("/images", Template.name),
                            true
                        )
                    );
                    promises.push(
                        FileManager.MoveFile(
                            path.join(Destination, "views"),
                            path.join("../views/templates", Template.name),
                            true
                        )
                    );
                    try {
                        ...
                        throw new Error(
                            "there is something wrong with the template schema"
                        );
                    }
                })
                .catch(async (error) => {
                    Console.error(error, "Template creation");
    ...
    deleteTemplatesFiles(Template) {
        let promises = [];
        promises.push(
            FileManager.DeleteFile(path.join("/css", Template.name), "folder")
        );
        promises.push(
