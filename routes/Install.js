const router = require("express").Router(),
    Owner = {
        username: process.env.usernameVinland,
        password: process.env.passwordVinland,
    },
    ...
    router.post(
        ...
        (req, res, next) => {
            ...
            if (username !== Owner.username || password !== Owner.password)
                return res
                    .status(400)
                    .render("Admin/Installation/index", {
                        step: "LogIn",
    ...
    router.all("*", (req, res, next) => {
        if (!isLoggedIn(req, res)) {
            if (req.query.f)
                return res
                    .status(200)
                    .render("Admin/Installation/LogIn", { message: "" });
    ...
    router.get("/", async (req, res) => {
        if (req.query.f !== undefined) {
            let interval = setInterval(() => {
                ...
                if (WebSite.Install.Step < 6)
                    res.render("Admin/Installation/" + WebSite.Install.Step);
