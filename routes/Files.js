...
router
    ...
    .get((req, res) => {
        let callback =
            req.query.treeView === "true"
                ? FilesManager.GetFilesTree
                : FilesManager.GetFolder;
    ...
    .put((req, res) => {
        ...
        } else if (req.body.NewName && req.body.NewName !== "index.html")
            FilesManager.MoveFile(
                req.URL + "/" + req.body.Name,
                req.URL + "/" + req.body.NewName
            )
                .then(() => {
                    res.status(200).send(
    ...
    .delete((req, res) => {
        if (req.body.Paths)
            FilesManager.DeleteFiles(req.body.Paths, (status, result) => {
                res.status(status).send(result);
            });
        else res.status(406).send("Bad Request");
    });
