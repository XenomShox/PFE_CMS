    ...
    .post((req, res) => {
        if (req.body.Name !== undefined) {
            if (!/^[1-9A-Za-z_]+$/.test(req.body.Name)) {
                res.status(401).send({
                    Error:
                        'Invalid Schema Name it must follow this regEX : "^[1-9A-Za-z]+$"',
                });
            } else if (req.body.Schema !== undefined) {
                ApiManager.Schemas.AddASchema(
                    req.body.Name,
                    req.body.Schema,
                    (status, result) => {
                        res.status(status).send(result);
            ...
            res.status(406).send({
                Error: "Can't Create a Schema without a Name",
            });
        }
    })
    .put((req, res) => {
        if (req.body.Name !== undefined) {
            if (!/^[1-9A-Za-z_]+$/.test(req.body.NewName)) {
                ...
                ApiManager.Schemas.UpdateASchema(
                    req.body.Name,
                    req.body.NewName,
                    req.body.Schema,
                    (status, result) => {
            ...
            res.status(406).send({
                Error: "Can't Update a Schema without a Name",
            });
        }
    })
    .delete((req, res) => {
        if (req.body.Name !== undefined) {
            ApiManager.Schemas.DeleteASchema(
                req.body.Name,
                (status, result) => {
                    res.status(status).send(result);
                }
            ...
            res.status(406).send({
                Error: "Can't Delete a Schema without a Name",
            });
        }
    });
/*------------------------------Data----------------------------*/
