...
router.route("/:Admin")
    .get((req, res) => {
        if (req.query.f) res.render("Admin/" + req.params.Admin, (err, html) => {
                if (err)res.status(404).render("Admin/error", {
...
router.route("/templates/route")
    ...
    .put((req,res)=>{
        TemplatesManager.applyTemplate(req.body.id)
            .then(()=>{ res.status(200).send("Template is applied") })
            .catch(reason => { res.status(500).send(reason.message) })
    })
    .delete((req, res) => {
        TemplatesManager.removeTemplate(req.body.id)
            .then(()=>{ res.status(200).send("Template has been deleted") })
            .catch(reason => { res.status(500).send(reason.message) })
    })
router.route("/templates/:id")
    .get(async (req, res) => {
        try{
            if(req.query.load) return FileManager.ReadFile(req.query.load)
                    .then(file=>{
                        res.status(200).send(file);
                    })
            let template= await TemplatesManager.getTemplate(req.params.id);
            if(req.query.tree){
                return FileManager.GetFilesTree(`/${req.query.tree}/${template.name}`,(status,result)=>{
                    res.status(status).send(result);
                },false);
            }
            if (req.query.f) return res.render("template/Edit",{template},(err, html) => {
                if(err) throw err;
                res.status(200).send(html);
            });
            res.render("Admin/index", { url: "/Admin/templates/" + req.params.id },(err, html) => {
                if(err) throw err;
                res.status(200).send(html);
            });
        }
    ...
    .put((req, res, next) => {
        try{
            if(req.query.update) return FileManager.WriteFile(req.query.update,req.body.data)
                .then(()=>{
