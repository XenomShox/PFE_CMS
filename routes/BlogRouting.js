...
router.get("/", async function (req, res) {
    try {
        ...
        res.render(
            `templates/${req.Schema.name}/${req.Schema.Index}`,
            { Posts },
            renderFunction(res)
        );
    } catch (e) {
...
router
    ...
    .put(isLoggedIn, hasPermission(["admin"]), (req, res) => {
        try {
            if (req.query.id) {
                CategoriesManager.UpdateCategory(req.query.id, req.body).then(
                    () => {
                        res.status(200).send({
    ...
    .delete(isLoggedIn, hasPermission(["admin"]), (req, res) => {
        try {
            if (req.query.id) {
                CategoriesManager.DeleteCategory(req.query.id).then(() => {
                    res.status(200).send("Done removing");
                });
            } else {
                throw new Error("there is no id");
            }
...
router
    ...
    .all(async (req, res, next) => {
        try {
            res.locals.Category = await CategoriesManager.GetCategory(
...
/*----------------Tag System-----------------*/
router.route("/tags/:tag").get(async (req, res) => {
    ...
    try {
        ...
        res.status(200).render(
            `templates/${req.Schema.name}/${req.Schema.Tags}`,
            { Posts, tag: req.params.tag },
            renderFunction(res)
        );
    } catch (e) {
        ErrorHandler(req, res, e);
...
router.route("/search/").get(async (req, res) => {
    try {
        ...
        res.render(
            `templates/${req.Schema.name}/${req.Schema.Search}`,
            {
                Posts,
                search,
            },
...
async function CategoryHandler(req, res, next) {
    if (req.query.post || req.query.create || req.query.edit) return next();
    res.locals.WebSite.Title += " - " + res.locals.Category.Name;
    try {
        res.locals.Posts = await PostManager.GetPosts({
            category: res.locals.Category._id,
        });
        res.status(200).render(
            `templates/${req.Schema.name}/${req.Schema.Categories}`,
            renderFunction(res)
        );
    } catch (e) {
        ErrorHandler(req, res, e);
...
function CreateHandler(req, res) {
    ...
    try {
        res.render(
            `templates/${req.Schema.name}/${req.Schema.CreatePost}`,
            renderFunction(res)
        );
    } catch (e) {
        ErrorHandler(req, res, e);
...
async function RatePost(req, res) {
    try {
        await PostManager.RatePost(
            req.query.post,
            /*req.user._id*/ "5f60162e8c5d9d18305ec7f7",
            req.body.rate
        );
...
async function EditPost(req, res, next) {
    ...
    if (req.body.covers && !(req.body.covers instanceof Array))
...
function ErrorHandler(req, res, e) {
    ...
    res.status(500).render(`templates/${req.Schema.name}/${req.Schema.Error}`, {
        message: e.message,
    });
}
