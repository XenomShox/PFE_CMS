...
router.get("/", function (req, res) {
    PostManager.GetPosts({ sort: "-visited" }, (status, Posts) => {
        if (status === 200)
            res.render(Schema.path + Schema.Index.path, { Posts });
    ...
    .all((req, res, next) => {
        ...
        CategoriesManager.GetCategory(req.URL, (status, category) => {
            if (status === 200) {
                ...
                res.status(404).render(Schema.path + Schema.Error.path, {
        ...
        if (req.query.post) {
            ...
            PostManager.CreatePost(
                ...
                (status, post) => {
                    if (status === 201)
                        res.redirect(
                            res.locals.Category.Slug + "?post=" + post._id
                        );
                    else
                        res.status(status).render(
                            Schema.path + Schema.Error.path,
...
router.route("/tags/:tag").get((req, res) => {
    ...
    PostManager.GetPosts({ tag: req.params.tag, ...query }, (status, Posts) => {
        if (status === 200)
            res.status(200).render(Schema.path + Schema.Tags.path, {
                Posts,
                tag: req.params.tag,
            });
        else
            res.status(status).render(Schema.path + Schema.Error.path, {
                message: Posts,
            });
    });
});
...
router
    ...
    .get((req, res, next) => {
        PostManager.GetPosts(
            ...
            (status, Posts) => {
                if (status === 200)
                    res.status(200).render(Schema.path + Schema.Search.path, {
                        Posts,
                        search:
                            req.query.search ||
                            req.query.category ||
...
function CategoryHandler(req, res) {
    ...
    PostManager.GetPosts({ category, ...req.query }, (status, Posts) => {
        if (status === 200)
            res.status(200).render(Schema.path + Schema.Categories.path, {
                Posts,
            });
        else
            res.status(status).render(Schema.path + Schema.Error.path, {
...
function PostHandler(req, res) {
    ...
    PostManager.GetPost(req.query.post, (status, Post) => {
        if (status === 200) {
            ...
            if (req.query.edit)
                res.render(Schema.path + Schema.EditPost.path, { Post });
